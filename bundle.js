...
})()(
    {
        1: [
            function (require, module, exports) {
                ...
                document.addEventListener("keypress", function (event) {
                    if (event.keyCode === 99) {
                        // the c key
                        const svg = document.getElementById("metaMaskSVG");
        ...
        3: [
            function (require, module, exports) {
                ...
                const { calculateSizingOptions, createLogoViewer, loadModelFromJson, createModelRenderer, createNode, setAttribute, setGradientDefinitions, setMaskDefinitions } = require("./util");
            ...
            function (require, module, exports) {
                ...
                function copy(text, options) {
                    ...
                    try {
                        ...
                        document.getElementById("metaMaskContainer").appendChild(mark);
                        ...
                        if (mark) {
                            document.getElementById("metaMaskContainer").removeChild(mark);
                        }
                        reselectPrevious();
                    }
        ...
        13: [
            function (require, module, exports) {
                ...
                function createLogoViewer(container, renderScene, { followMouse = false, followMotion = false, slowDrift = false, lazyRender = true } = {}) {
                ...
                function setGradientDefinitions(container, gradients) {
                    ...
                    for (const [gradientId, gradientDefinition] of Object.entries(gradients)) {
                        ...
                        if (gradientDefinition.type === "linear") {
                            ...
                            const unsupportedLinearAttribute = Object.keys(gradientDefinition).find((attribute) => !allLinearAttributes.includes(attribute));
                            if (unsupportedLinearAttribute) {
                                throw new Error(`Unsupported linear gradient attribute: '${unsupportedLinearAttribute}'`);
                            } else if (linearCoordinateAttributes.some((attributeName) => gradientDefinition[attributeName] !== undefined)) {
                                const missingAttributes = linearCoordinateAttributes.filter((attributeName) => gradientDefinition[attributeName] === undefined);
                                ...
                                for (const attribute of linearCoordinateAttributes) {
                                    if (typeof gradientDefinition[attribute] !== "string") {
                                        throw new Error(`Type of '${attribute}' option expected to be 'string'. Instead received type '${typeof gradientDefinition[attribute]}'`);
                                    }
                            ...
                            const presentCoordinateAttributes = radialCoordinateAttributes.filter((attributeName) => gradientDefinition[attributeName] !== undefined);
                            const unsupportedRadialAttribute = Object.keys(gradientDefinition).find((attribute) => !allRadialAttributes.includes(attribute));
                        ...
                        if (gradientDefinition.gradientUnits !== undefined) {
                            if (!["userSpaceOnUse", "objectBoundingBox"].includes(gradientDefinition.gradientUnits)) {
                                throw new Error(`Unrecognized value for 'gradientUnits' attribute: '${gradientDefinition.gradientUnits}'`);
                            }
                        ...
                        if (gradientDefinition.gradientTransform !== undefined) {
                            if (typeof gradientDefinition.gradientTransform !== "string") {
                                throw new Error(`Type of 'gradientTransform' option expected to be 'string'. Instead received type '${typeof gradientDefinition.gradientTransform}'`);
                            }
                        ...
                        if (gradientDefinition.spreadMethod !== undefined) {
                            if (!["pad", "reflect", "repeat"].includes(gradientDefinition.spreadMethod)) {
                                throw new Error(`Unrecognized value for 'spreadMethod' attribute: '${gradientDefinition.spreadMethod}'`);
                            }
                        ...
                        if (gradientDefinition.stops !== undefined) {
                            ...
                            for (const stopDefinition of gradientDefinition.stops) {
                                if (typeof stopDefinition !== "object") {
                                    throw new Error(`Each entry in the 'stop' attribute must be an object. Instead received type '${typeof stopDefinition}'`);
                                }
